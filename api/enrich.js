export default async function handler(req, res) {
  ...
  let scraped = { email: '', linkedin: '', phone: '', address: '', html_text: '' };
  ...
  if (website) {
    try {
      ...
      // Emails - filter out generic/system ones
      const genericDomains = ['gmail','yahoo','hotmail','outlook','example','sentry','wix','wp','noreply'];
      ...
      // Phone numbers
      const phoneMatch = html.match(/(\+?[\d][\d\s\-().]{7,18}[\d])/g);
      const cleanPhones = (phoneMatch || [])
        .map(p => p.trim())
  ...
  try {
    const groqRes = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `You are a B2B healthcare CRM data specialist. Analyze this company and fill ALL fields possible.

Company name: ${name}
Website: ${website || 'unknown'}
Website content excerpt: ${scraped.html_text || 'not available'}
Found email: ${scraped.email || 'none'}
Found phone: ${scraped.phone || 'none'}

Return ONLY valid JSON (no markdown, no explanation):
{
  "industry": <11=Hospitals & Health Care, 14=Professional Services, 17=Technology, 12=Manufacturing, 16=Retail, 18=Transportation>,
  "annual_revenue": <1=under 1M, 2=1-10M USD, 3=10-100M USD, 4=100-1000M USD, 5=1-10B USD>,
  "icp": <64=Yes, 65=No, 371=No-too small, 66=?>,
  "ownership": <1014=Private, 1015=Public, 1016=Unknown>,
  "icp_type": <1017=Hospital, 935=Clinic/Polyclinic, 1018=Specialist Practice, 520=Dental Clinic, 932=Diagnostic Center & Laboratory, 939=HIS Software Provider, 937=Nursing Home / Long-term Care, 936=Dialysis Clinic, 1019=Physiotherapy Clinic, 1020=Aesthetic & Plastic Surgery Clinic, 1021=Ophthalmology Clinic, 1022=Radiology Center, 1023=Rehabilitation Center, 1024=Mental Health Clinic, 934=Maternity and IVF Clinic, 931=Home Care, 1028=Unknown>,
  "qualify_status": <62=Qualified with contact, 63=Qualified with contact+email, 61=Qualified no contact, 57=To qualify>,
  "org_source": 546,
  "employees_category": <49=1-10, 50=11-50, 51=51-200, 52=201-500, 53=501-1000, 54=1001-5000>,
  "icp_ecosystem": <854=Healthcare ecosystem, 855=Healthcare software vendors (partnership)>,
  "his_identification": <850=Yes, 851=No>,
  "company_legal_name": "full legal name with legal form (Ltd, GmbH, UAB, S.A. etc) or empty string",
  "his_software_name": "name of HIS/RIS/LIS/PACS software they use or sell, or empty string",
  "ceo_name": "CEO or founder full name if determinable, or empty string",
  "address": "full address if found on website or determinable, or empty string",
  "number_of_beds": 0,
  ...
  const finalLinkedin = scraped.linkedin || '';
  const finalAddress = enriched.address || '';

  const payload = {};
  const set = (key, val) => { if (val !== undefined && val !== null && val !== '' && val !== 0) payload[key] = val; };
  ...
  if (finalPhone) payload.phone = [{ value: finalPhone, primary: true }];
  ...
  set('b4c2b2ef4b92a130ec7de91f4d17622d5640431e', enriched.company_legal_name); // Company legal name
  set('115cfff712c6caf184d7c155838a9dace81e8821', enriched.his_software_name);  // HIS software
  set('4107458f7b06285686f1968fbefa9ea50902cf07', enriched.ceo_name);           // CEO for contract
  set('6d64ec2abf8d9a01a64c0cbf2f962281845b1c85', finalAddress);                // Address for license
  ...
  if (!pdData.success) console.error('Pipedrive error:', pdData);

  // Add note with company overview
